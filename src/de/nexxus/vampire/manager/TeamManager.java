...
public class TeamManager {
  ...
  private static Scoreboard scoreboard = Bukkit.getScoreboardManager().getNewScoreboard(),
      survivorSB = Bukkit.getScoreboardManager().getNewScoreboard();
  private static Team vampiresTeam = scoreboard.registerNewTeam(Teams.VAMPIRE.name()),
      survivorTeam = survivorSB.registerNewTeam(Teams.SURVIVOR.name());
  private static Objective
      vampiresObjective = scoreboard.registerNewObjective("vampireScore", "dummy"),
      survivorObjective = survivorSB.registerNewObjective("survivorScore", "dummy");
  private static HashMap<Player, TeamManager> teamManagerHashMap = new HashMap<>();
  public static Player vampire;
  private Player player;
  private Teams team;
  public static List<Player> vampires = Lists.newArrayList(),
  ...
  public static Teams getPlayerTeam(Player player) {
    if (player == getVampire()) return Teams.VAMPIRE;

    if (survivors.contains(player)) return Teams.SURVIVOR;
    if (spectators.contains(player)) return Teams.SPECTATOR;
    return null;
  }
}
